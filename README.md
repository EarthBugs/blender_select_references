...
| 你选中的 | 触发后选中的 | 原因 |
|---|---|---|
| 贴图（Image） | 引用它的**材质 / 摄像机**（大纲行高亮）；若引用者是**图像空物体**（Image Empty，即视口中用于显示图片的 Empty），则选中该空物体**对象** | 节点树 / 背景图 / Empty 引用 |
...
若选中的数据块没有任何引用者（例如一张从没被用到的贴图），结果为空——状态栏仅提示"没有找到直接引用所选数据块的对象或数据块"，不会选中任何东西。
